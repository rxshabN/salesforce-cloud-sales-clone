  ...
  return (
    <div className="h-full flex flex-col bg-[#f3f2f2]">
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex items-center gap-1 pb-3 font-normal transition-colors relative ${
                    activeTab === tab.id ? "text-[#0176d3]" : "text-gray-700 hover:text-[#0176d3]"
                  }`}
                >
                  {tab.label}
                  <ChevronDown className="w-4 h-4" />
                  {activeTab === tab.id && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#0176d3]" />}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeTab === "products" && (
          <div className="p-8 text-center text-gray-600">Products content coming soon...</div>
        )}
        {activeTab === "price-books" && (
          <div className="p-8 text-center text-gray-600">Price Books content coming soon...</div>
        )}
        {activeTab === "calendar" && (
          <div className="p-8 text-center text-gray-600">Calendar content coming soon...</div>
        )}
        {activeTab === "analytics" && (
          <div className="p-8 text-center text-gray-600">Analytics content coming soon...</div>
        )}
      </div>
    </div>
  )
